...
public class ChangePasswordWithPasswordIntegrationTest extends IntegrationBaseTest {

  @Autowired
  private JpaUserRepository jpaUserRepository;

  @Test
  @DisplayName("Integración: Usuario cambia password con la anterior y puede iniciar sesión")
  void shouldChangePasswordWithOldPasswordAndLogin() {
    String adminToken = login("super_admin@example.mail", "super");

    String unique = UUID.randomUUID().toString().replace("-", "");
    String email = "change_pass_old_" + unique + "@example.mail";
    String initialPassword = "Abcd1234";
    ...
    ChangePasswordWithTokenRequest configureRequest = new ChangePasswordWithTokenRequest(token, initialPassword);
    webTestClient.patch()
      .uri("/api/auth/change-password-with-token")
      .contentType(MediaType.APPLICATION_JSON)
      .bodyValue(configureRequest)
      .exchange()
      .expectStatus().isNoContent();

    String userToken = login(email, initialPassword);

    ChangePasswordWithPasswordRequest changeRequest = new ChangePasswordWithPasswordRequest(initialPassword, newPassword);
    webTestClient.patch()
      .uri("/api/auth/change-password-with-password")
